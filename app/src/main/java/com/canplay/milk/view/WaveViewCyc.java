 ...
 */
public class WaveViewCyc extends View {
    ...
    public WaveViewCyc(Context context) {
        super(context);
        initView(null);
    }

    public WaveViewCyc(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(attrs);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        firstPath.reset();
        secondPath.reset();
        bitmapCanvas.drawCircle(waveSize / 2, waveSize / 2, waveSize / 2, backgroundPaint);        /**水位线*/
     ...
     */
    public WaveViewCyc setWaterProgress(float waterProgress) {
        if (waterProgress <= 0) {
            waterProgress = 0;
     ...
     * 设置第一条波浪线的 颜色
     */
    public WaveViewCyc setFirstWaveColor(int firstWaveColor) {
     ...
     * 设置第二条波浪线的 颜色
     */
    public WaveViewCyc setSecondWaveColor(int secondWaveColor) {
     ...
     * 设置 波动速读
     */
    public WaveViewCyc setSpeed(int speed) {
        if (speed < 1) {
            speed = 1;
        }
        if (speed > 10) {
     ...
     */
    public WaveViewCyc setAmplitude(int amplitude) {
        if (amplitude <= 0) {
            amplitude = 0;
        }
